...
5. `Assets/GameSelect/GameSelectPanel.prefab` をワールドへ配置します。新規生成は `Game Select > Create Panel in Scene`。
...
変更はUnity編集時に反映します。公開済みVRChatワールドへの反映には再ビルド・アップロードが必要です。各プレイヤーのローカル動作で、ネットワーク同期や永続保存は未実装です。クリッカーの進行は同じセッション中のみ保持します。
